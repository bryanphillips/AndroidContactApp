...
namespace ContactApp
{
	...
	public class ImageLoader
	{
		...
		{
			cache = new LRUCache<Uri, Drawable> (cacheSize, memoryLimit, sizer);
            StringBuilder builder = new StringBuilder();
            if (Android.OS.Environment.ExternalStorageState.Equals(Android.OS.Environment.MediaMounted))
            {
                //storage device is sd card.
                builder.Append(Android.OS.Environment.ExternalStorageDirectory.AbsolutePath);
                builder.Append("/Android/data");
                if (!string.IsNullOrEmpty(CurrentActivity.ApplicationContext.PackageName))
                {
                    builder.Append("/");
                    builder.Append(CurrentActivity.ApplicationContext.PackageName);
                }
                builder.Append("/cache/");
            }
            else
            {
                builder.Append(CurrentActivity.ApplicationContext.CacheDir.AbsolutePath);
            }
            PicDir = builder.ToString();

            if (!Directory.Exists(PicDir))
                Directory.CreateDirectory(PicDir);
		}

		static int sizer (Drawable img)
		{
            var bitmap = ((BitmapDrawable)img).Bitmap;
		...
		/// </returns>
		public static Drawable DefaultRequestImage (Uri uri, IImageUpdated notify)
		{
			if (DefaultLoader == null)
		...
		/// If the image has already been downloaded, or is in the cache, this will return the image as a UIImage.
		/// </returns>
		public Drawable RequestImage (Uri uri, IImageUpdated notify)
		{
			Drawable ret;
			...
			if (File.Exists (picfile)){
                ...
                ret = Drawable.CreateFromPath(picfile);
				if (ret != null){
					lock (cache)
						cache [uri] = ret;
					return ret;
